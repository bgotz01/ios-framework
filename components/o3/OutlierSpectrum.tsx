//components/o3/OutlierSpectrum.tsx
"use client";
...
function getRarityLabel(rarity: number) {
    ...
    if (rarity < 80) return "Extreme";
    if (rarity < 95) return "Singular";
    return "Absolute Outlier";
}
...
export default function OutlierSpectrum({
    example,
    initialLevelId,
}: Props) {
    ...
    const active = useMemo(() => {
        return (
            example.levels.find((level) => level.id === activeId) ??
            example.levels[example.levels.length - 1]
        );
    }, [activeId, example.levels]);

    const activeIndex = example.levels.findIndex((level) => level.id === active.id);
        ...
        <section className="w-full rounded-[28px] border border-white/10 bg-black text-white shadow-2xl">
            ...
            <div className="px-4 py-6 md:px-6 md:py-8">
                <div className="overflow-x-auto pb-3">
                    <div
                        style={{
                            minWidth: `${Math.max(example.levels.length * 90, 640)}px`,
                        }}
                    >
                        {/* Axis — Common → Rare */}
                                ...
                                {example.levels.map((level, index) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex flex-col items-center text-center outline-none rounded-lg px-1 py-1.5 transition-colors ${isHovered ? "bg-white/10" : ""}`}
                                        >
                                            <div
                        ...
                        <div className="relative px-2 pt-6">
                            ...
                            >
                                {example.levels.map((level, index) => {
                                    ...
                                    const nodeSize = clamp(16 + level.rarity * 0.28, 18, 44);
                                    const glowSize = clamp(34 + level.rarity * 0.45, 38, 78);
                                    const barHeight = clamp(getFrequency(level), 4, 92);
                                    ...
                                    return (
                                        ...
                                        >
                                            <motion.div
                                                ...
                                                className={`mb-3 w-3 rounded-full ${isActive
                                                    ? "bg-white"
                                                    : isPastOrActive
                                                        ? "bg-white/55"
                                                        : "bg-white/20"
                                            ...
                                            {index < example.levels.length - 1 && (
                                                <ChevronRight className="absolute right-[-11px] top-[78px] h-4 w-4 text-white/20" />
                                            )}
                                        </button>
                                    );
                                })}
